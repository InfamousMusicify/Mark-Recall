...
execute at @s[scores={mark=1..,mark_track=0,mark_crouch=100..}] as @e[sort=nearest,limit=1,scores={mark_tag_along=1..},distance=1..] if score @s mark_tag_along = @a[sort=nearest,limit=1,scores={mark_track=0,mark_crouch=100..},distance=1..] markrec_id run scoreboard players reset @s mark_tag_along
# mark people within 8 blocks
execute at @s[scores={mark=1..,mark_track=0,mark_crouch=..99}] store result score @e[sort=nearest,limit=1,distance=1..8] mark_tag_along run scoreboard players get @s markrec_id
