...
execute if score #limit homes.config < @s[scores={mark_crouch=0}] mark_track run scoreboard players set @s[scores={mark_crouch=0}] mark_track 0

# msg
tellraw @s[scores={mark_crouch=0}] [{"text":"Home: ","color":"gold"},{"score":{"name":"@s","objective":"mark_track"},"color":"red"}]
# reset cycle
