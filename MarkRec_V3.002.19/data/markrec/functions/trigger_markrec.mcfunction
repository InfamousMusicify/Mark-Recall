...
execute as @s[scores={recall=1..,mark_track=0,mark_crouch=0}] as @e[scores={mark_tag_along=1..},distance=1..] at @a[sort=nearest,scores={recall=1..}] if score @s mark_tag_along = @a[sort=nearest,limit=1,scores={recall=1..}] markrec_id run tp @s ~ ~ ~
